...
describe('<TagSelector />', () => {
  ...
  describe('input is not empty', () => {
    const tagCaption = 'Example';
    let action;

    beforeEach(() => {
      input.simulate('change', { target: { value: tagCaption } });
    });
    ...
    describe('hitting komma', () => {
      ...
      it('should dispatch the submitted tag', () => {
        expect(action.tag.caption).toEqual(tagCaption);
      });
    });
  ...
  it('should display a return hint if there is exactly one tag', () => {
    wrapper.setProps({ tags: [TagFactory.createDummyTag()] });
